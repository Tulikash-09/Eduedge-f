...
export default function TutorPage() {
  ...
  return (
    <div className="flex flex-col h-screen">
      {/* ── Header ── */}
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/[0.07] bg-card flex-shrink-0">
        <div>
          <h1 className="font-display text-lg font-extrabold text-slate-100">AI Tutor</h1>
          <div className="flex items-center gap-2 mt-1">
            <span className="pill pill-blue">CBSE Class 10</span>
            <span className="text-slate-500 text-xs">Science · Socratic Method</span>
          </div>
        </div>
      ...
      <div className="px-6 py-4 border-t border-white/[0.07] bg-card flex gap-3 items-center flex-shrink-0">
        <input
          ...
          placeholder={
            lang === "hi"
              ...
              : "Ask any CBSE Class 10 Science question... (Enter to send)"
          }
        />
        <button
          onClick={sendMessage}
        ...
        >
          {loading ? <Spinner size={16} /> : <span className="text-lg">↑</span>}
        </button>
      </div>
    </div>
  );
